...
# personal librairies
sys.path.append('D:\3A\TLD\scripts')
...
image_filename = os.path.join(my_folder, 'chemin.tif') #Image à 60 bandes

# Rasterize each level from polygons
...
# Create a raster with groups of each polygon
gdf = gpd.read_file(sample_filename)
gdf['poly_id'] = range(1, len(gdf) + 1)
gdf.to_file(os.path.join(my_folder,'code_group.shp'))
out_name = os.path.join(my_folder,'code_group.tif')
cmd = cmd_pattern.format(in_vector=sample_filename, xmin=xmin, ymin=ymin, 
                             xmax=xmax, ymax=ymax, out_image=out_name, 
                             field_name="poly_id",
                             sptial_resolution=sptial_resolution)
"""
# Sample parameters
